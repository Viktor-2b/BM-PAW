...
# 计算诚实挖矿策略的相对奖励的函数
def calculate_rr_honest_rum(alpha, p_attack):
    return (1 - alpha - p_attack * alpha) / (1 - p_attack * alpha)

def calculate_rr_honest_uum(alpha, p_attack):
    return (1 - alpha - p_attack * alpha) / (1 - p_attack * alpha)

def calculate_rr_honest_suum(alpha, p_attack1, p_deploy):
    denominator = alpha + (1 - p_attack1) * (1 - alpha) - (1 - p_deploy) * alpha
    numerator = (1 - p_attack1) * (1 - alpha) - (1 - p_deploy) * alpha
    return numerator / denominator

# 计算诚实挖矿的相对奖励
def calculate_rr_honest(alpha):
    return 1 - alpha  # 诚实挖矿的相对奖励为1-alpha
...
for alpha in alphas:
    ...
    rr_honest_rum.append(calculate_rr_honest_rum(alpha, p_attack_rum))
    
    # UUM
    result_uum = markov_monte_carlo_uum(alpha, N)
    p_attack_uum = result_uum[1]  # UUM的Attack状态概率
    rr_honest_uum.append(calculate_rr_honest_uum(alpha, p_attack_uum))
    ...
    rr_honest_suum.append(calculate_rr_honest_suum(alpha, p_attack1_suum, p_downgrade_suum))
...
# 绘制曲线，使用不同的线型和标记
ax.plot(alphas, rr_honest_rum, 'b-', label='RUM', linewidth=3, marker='o', markersize=10, markevery=10)  # 实线，圆形标记
ax.plot(alphas, rr_honest_uum, 'r--', label='UUM', linewidth=3, marker='s', markersize=10, markevery=10)  # 虚线，方形标记
ax.plot(alphas, rr_honest_suum, 'g:', label='SUUM', linewidth=3, marker='^', markersize=10, markevery=10)  # 点线，三角形标记
ax.plot(alphas, rr_honest, 'k-.', label='Honest Mining', linewidth=3, marker='d', markersize=10, markevery=10)  # 点划线，菱形标记

# 设置坐标轴标签
ax.set_xlabel('Adversary Relative Power $\\alpha$', fontsize=20)
ax.set_ylabel('$RR_{Honest}$', fontsize=20)

# 设置标题
ax.set_title('Comparison of $RR_{Honest}$', fontsize=20, pad=20)
...
# 保存图像
plt.savefig('honest_reward_comparison.pdf', dpi=300, bbox_inches='tight')
